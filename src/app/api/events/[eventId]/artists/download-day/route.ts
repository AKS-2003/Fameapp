...
export async function POST(
	...
	try {
		...
		const { artistIds, targets, date, dayNumber, eventName } = body;

		const zip = new JSZip();
		const dayFolder = zip.folder(
			`Day_${dayNumber}_${new Date(date).toLocaleDateString("en-US", {
				month: "short",
				day: "numeric",
			})}`,
		);

		// Helper function to read file from VPS local storage
		...
		// Map to a list of `{ artistId, eventShowId }` targets
		const itemsToProcess = targets && Array.isArray(targets) 
			? targets 
			: (artistIds || []).map((id: string) => ({ artistId: id }));

		// Process all artists in parallel
		await Promise.all(
			itemsToProcess.map(async ({ artistId, eventShowId }: { artistId: string; eventShowId?: string }, index: number) => {
				try {
					...
					const artistFolder = dayFolder?.folder(
		...
		return new NextResponse(zipBuffer, {
			...
			headers: {
				...
				"Content-Disposition": `attachment; filename="Day_${dayNumber}_${new Date(
					date,
				).toLocaleDateString("en-US", {
					month: "short",
					day: "numeric",
				})}_Artists_${eventName}.zip"`,
		...
		console.error("Error generating day download:", error);
		return NextResponse.json(
			{ error: "Failed to generate day download" },
			{ status: 500 },
		);
	}
}
