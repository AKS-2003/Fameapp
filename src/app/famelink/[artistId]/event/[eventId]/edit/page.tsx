...
export default function EditEventCopyPage() {
	...
	const eventShowId = searchParams.get("eventShowId") || "";
	...
	const originalMusicTrackRef = useRef<any>(null);
	...
	// Fetch EventShow data — merge snapshotJson + overrides (overrides win)
	useEffect(() => {
		const fetchEventShow = async () => {
			if (!eventShowId) {
				toast({
					title: "Error",
					description: "Missing eventShowId parameter",
					variant: "destructive",
				});
			...
			try {
				const response = await fetch(`/api/event-shows/${eventShowId}`);
				const result = await response.json();
				if (result.success && result.data?.eventShow) {
					const es = result.data.eventShow;
					const snap = es.snapshotJson || {};
					const ov = es.overrides || {};

					// Merge: overrides take precedence over snapshot
					const newFormData = {
						name: ov.name ?? snap.name ?? "",
						real_name: ov.realName ?? snap.realName ?? "",
						email: ov.email ?? snap.email ?? "",
						phone: ov.phone ?? snap.phone ?? "",
						managed_by: ov.managedBy ?? snap.managedBy ?? "",
						style: ov.style ?? snap.style ?? "",
						performance_type:
							ov.performanceType ?? snap.performanceType ?? "",
						biography:
							ov.biography ??
							snap.biography ??
							snap.description ??
							"",
						notes: ov.notes ?? snap.notes ?? "",
						props_needed:
							ov.propsNeeded ??
							snap.equipment ??
							snap.techRider ??
							"",
						performance_duration: ov.duration ?? snap.duration ?? 5,
						manual_costume_color:
							ov.manualCostumeColor ??
							snap.manualCostumeColor ??
							"",
						manual_costume_color_two:
							ov.manualCostumeColorTwo ??
							snap.manualCostumeColorTwo ??
							"",
						manual_costume_color_three:
							ov.manualCostumeColorThree ??
							snap.manualCostumeColorThree ??
							"",
						light_requests:
							ov.lightRequests ?? snap.lightRequests ?? "",
						manual_light_color:
							ov.manualLightColor ?? snap.manualLightColor ?? "",
						manual_light_color_two:
							ov.manualLightColorTwo ??
							snap.manualLightColorTwo ??
							"",
						manual_light_color_three:
							ov.manualLightColorThree ??
							snap.manualLightColorThree ??
							"",
						show_link: ov.showLink ?? snap.showLink ?? "",
						stage_position_start:
							ov.stagePositionStart ??
							snap.stagePositionStart ??
							"",
						stage_position_end:
							ov.stagePositionEnd ?? snap.stagePositionEnd ?? "",
						custom_stage_position:
							ov.customStagePosition ??
							snap.customStagePosition ??
							"",
						mc_notes: ov.mcNotes ?? snap.mcNotes ?? "",
						stage_manager_notes:
							ov.stageManagerNotes ??
							snap.stageManagerNotes ??
							"",
						instagram_link:
							ov.socialMedia?.instagram ??
							snap.socialMedia?.instagram ??
							"",
						facebook_link:
							ov.socialMedia?.facebook ??
							snap.socialMedia?.facebook ??
							"",
						tiktok_link:
							ov.socialMedia?.tiktok ??
							snap.socialMedia?.tiktok ??
							"",
						youtube_link:
							ov.socialMedia?.youtube ??
							snap.socialMedia?.youtube ??
							"",
						website_link:
							ov.socialMedia?.website ??
							snap.socialMedia?.website ??
							"",
						country_living:
							ov.countryLiving ?? snap.countryLiving ?? "",
						home_country: ov.homeCountry ?? snap.homeCountry ?? "",
					};
					setFormData(newFormData);
					originalDataRef.current = newFormData;

					const mergedMembers = ov.members ?? snap.members;
					if (mergedMembers?.length) {
						setMembers(mergedMembers);
						originalMembersRef.current = mergedMembers;
					}

					const mergedTshirts = ov.tshirtSizes ?? snap.tshirtSizes;
					if (mergedTshirts?.length) {
						setTshirtSizes(mergedTshirts);
						originalTshirtSizesRef.current = mergedTshirts;
					}

					const mergedMusic = ov.musicTrack ?? snap.musicTrack;
					if (mergedMusic) {
						setMusicTrack(mergedMusic);
						originalMusicTrackRef.current = mergedMusic;
					}

					const mergedGallery = ov.galleryFiles ?? snap.galleryFiles;
					if (mergedGallery) {
						setGalleryFiles(mergedGallery);
						originalGalleryFilesRef.current = mergedGallery;
					}

					const mergedProfile = ov.profileImage ?? snap.profileImage;
					if (mergedProfile) {
						setProfileImage(mergedProfile);
						originalProfileImageRef.current = mergedProfile;
					}

					const mergedRehearsal =
						ov.rehearsalVideo ?? snap.rehearsalVideo;
					if (mergedRehearsal) {
						setRehearsalVideo(mergedRehearsal);
						originalRehearsalVideoRef.current = mergedRehearsal;
					}
				} else {
					toast({
						title: "Error",
						description: "Failed to load event show",
						variant: "destructive",
					});
					router.push(`/famelink/${artistId}/event/${eventId}`);
				}
		...
		fetchEventShow();
	}, [eventShowId, artistId, eventId, router, toast]);
	...
	const handleMusicUpload = async (
		...
		try {
			...
			toast({
				...
				description: "Music file uploaded",
				variant: "success",
			});
		} catch {
			toast({
	...
	// Save handler — sends ALL form values as overrides via PUT
	// CRITICAL: This ONLY writes to EventShow.overrides, NEVER to snapshotJson
		...
		try {
			const response = await fetch(`/api/event-shows/${eventShowId}`, {
				...
				body: JSON.stringify({
					overrides: {
						...
						propsNeeded: formData.props_needed,
			...
			if (result.success) {
				try {
					...
					if (orig) {
						// Compare form fields — normalize both sides to avoid false positives
						...
						for (const key of Object.keys(orig)) {
							const origVal = normalize(orig[key]);
							const newVal = normalize((formData as any)[key]);
							if (origVal !== newVal) {
					...
					if (
						musicTrack?.file_url !==
						originalMusicTrackRef.current?.file_url
					) {
						changedFields.push("music_tracks");
					}
					if (
						JSON.stringify(galleryFiles.map((f: any) => f.url)) !==
						JSON.stringify(
					...
					if (changedFields.length > 0) {
						...
						await fetch(`/api/events/${eventId}/artist-updates`, {
							...
							body: JSON.stringify({
								artistId,
								artistName: formData.name || "FameLink Artist",
								changedFields,
								summary,
							}),
						});
					...
					console.error("Error logging artist update:", notifError);
				}

				toast({
					title: "Event Copy Updated!",
					description: "Your event-specific changes have been saved",
					variant: "success",
				});
				router.push(
					`/famelink/${artistId}/event/${eventId}?eventShowId=${eventShowId}`,
				);
			} else {
				toast({
					title: "Error",
					description:
						result.error?.message || "Failed to update event copy",
					variant: "destructive",
				});
			}
		} catch {
			toast({
				title: "Error",
				description: "Failed to update event copy",
				variant: "destructive",
			});
		} finally {
			setSaving(false);
	...
	if (loading) {
		return (
			<div className="min-h-screen bg-gradient-to-br from-purple-900 via-purple-800 to-pink-900 text-white flex items-center justify-center">
		...
		<div className={isEmbedded ? "min-h-screen bg-white text-gray-900" : "min-h-screen bg-gradient-to-br from-purple-900 via-purple-800 to-pink-900 text-white"}>
						...
						<div className="flex items-center gap-4">
							...
							<div>
								...
								<p className="text-purple-100 text-sm">
									Edit your event-specific show details
								</p>
							</div>
						</div>
						<Link
							href={`/famelink/${artistId}/event/${eventId}${eventShowId ? `?eventShowId=${eventShowId}` : ""}`}
						>
							<Button
								variant="ghost"
			...
			<main className="container mx-auto px-4 py-8 max-w-5xl">
						...
						<div className="flex items-start gap-3">
							...
							<div>
								<p className="font-semibold text-orange-900">
									You are editing the event copy, not your master
									profile
								</p>
								<p className="text-sm text-orange-700 mt-1">
									Changes here only affect this specific event.
									Your original show profile remains unchanged. To
									update your master profile, go back to your
									dashboard and edit the show directly.
								</p>
							</div>
						</div>
					</div>
				...
				<div className="space-y-4">
					<fieldset disabled={isReadonly} className="w-full space-y-4 border-0 p-0 m-0">
					<Accordion
						type="single"
											...
											<div className="flex flex-col md:flex-row items-center gap-6">
												<div className="relative">
													...
													{uploadingProfileImage && (
														<div className="absolute inset-0 bg-black/60 rounded-full flex flex-col items-center justify-center p-2 text-center">
															<div className="animate-spin rounded-full h-8 w-8 border-b-2 border-white mb-2"></div>
															<div className="w-full px-2">
																<Progress value={profileImageProgress} className="h-1 bg-white/20" />
																<p className="text-[10px] text-white mt-1 font-bold">{profileImageProgress}%</p>
															</div>
														</div>
													)}
												</div>
												<div className="flex-1 w-full">
													<Label
														htmlFor="event-edit-profile-image"
												...
												!uploadingMusic && (
													<div className="border-2 border-dashed border-pink-300 rounded-lg p-6 hover:border-pink-400 hover:bg-pink-50/50 transition-all duration-300">
														<div className="text-center space-y-3">
															...
															<div>
																<Label
																	htmlFor="event-edit-music-upload"
									...
									<CardContent className="space-y-6 pt-6">
										...
										<div className="space-y-3">
											...
											{rehearsalVideo ? (
												...
												<div>
													<Label
														htmlFor="event-edit-rehearsal-upload"
														className="cursor-pointer block"
													>
													...
													<input
														id="event-edit-rehearsal-upload"
														type="file"
														accept="video/*"
														onChange={
										...
										<div className="space-y-4">
											...
											<Label
												htmlFor="event-edit-gallery-upload"
												className="cursor-pointer block"
											>
					...
					{/* Save Button */}
					{!isReadonly && (
						<div className="bg-gradient-to-r from-orange-50 to-purple-50 border-2 border-orange-200 rounded-2xl p-6 mt-6">
							<div className="flex items-center gap-3 mb-4">
								<h3 className="font-bold text-gray-900">
									Save Event Copy Changes?
								</h3>
								<p className="text-sm text-gray-600">
									These changes only affect this event.
								</p>
							</div>
							<div className="flex gap-4">
								...
								<Button
									onClick={handleSave}
									disabled={saving}
									className="flex-1 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white py-6 text-lg font-semibold"
								>
									{saving ? (
