...
interface ActInfo {
	...
	needs: LogisticsNeeds;
}

const EMPTY_NEEDS: LogisticsNeeds = { flights: null, hotel: null, transport: null, visa: null };
...
export default function LogisticsPage() {
	...
	// Act info
	const [actInfo, setActInfo] = useState<ActInfo>({ actName: "", leadContact: "", expectedTravelers: 0, needs: EMPTY_NEEDS });
				...
				if (result.success) {
					...
					if (dbLogistics) {
						...
						setActInfo({
							...
							leadContact: dbLogistics.leadContact || "",
							expectedTravelers: parseInt(dbLogistics.expectedTravelers) || 0,
							needs: dbLogistics.needs || EMPTY_NEEDS,
						});
	...
	// ── Render ───────────────────────────────────────────────────────
	return (
		<div className="min-h-screen bg-[#080414] text-white">
			{/* Ambient background effects */}
			<div className="fixed inset-0 pointer-events-none">
				<div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] rounded-full bg-purple-600/8 blur-[120px]" />
				<div className="absolute bottom-[-20%] right-[-10%] w-[500px] h-[500px] rounded-full bg-pink-600/8 blur-[120px]" />
			</div>

			{/* ── Top Navigation Bar ──────────────────────────────────────── */}
			<header
				className="relative z-20 border-b border-white/5"
				style={{ background: "rgba(15, 8, 35, 0.9)", backdropFilter: "blur(20px)" }}
			>
				<div className="container mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
					<Link
						href={`/famelink/${artistId}`}
						className="flex items-center gap-2 text-purple-300/70 hover:text-white transition-colors text-sm group"
					>
					...
					{hasProfile && (
						<div className="relative group">
							<button 
								...
								className="flex items-center gap-2 px-4 py-2 rounded-full border border-pink-500/30 bg-pink-500/10 hover:bg-pink-500/20 transition-colors"
							>
								<User className="h-4 w-4 text-pink-400" />
								<span className="text-sm font-medium text-pink-200">{artistName}</span>
								<ChevronDown className="h-3 w-3 text-pink-300/50" />
							</button>

							{isDropdownOpen && (
								<div className="absolute right-0 top-full mt-2 w-56 rounded-xl border border-white/10 shadow-xl z-50 overflow-hidden"
									 style={{ background: "rgba(15, 8, 35, 0.95)", backdropFilter: "blur(20px)" }}
								>
									<div className="p-2">
										<div className="px-3 py-2.5 text-sm font-medium text-pink-200 bg-pink-500/10 rounded-lg flex items-center justify-between cursor-default">
											{artistName}
											<Check className="h-4 w-4 text-pink-400" />
										</div>
										<div className="my-1.5 border-t border-white/5"></div>
										<button 
											className="w-full text-left px-3 py-2 text-sm font-bold text-pink-400 hover:bg-white/5 rounded-lg flex items-center gap-2 transition-colors"
											onClick={() => {
												setIsDropdownOpen(false);
												toast({ title: "Coming Soon", description: "Adding multiple logistics profiles will be available soon." });
											}}
										>
											<Plus className="h-4 w-4" /> Add Logistic
										</button>
									</div>
								</div>
							)}
			...
			<div className="h-px bg-gradient-to-r from-transparent via-purple-500/20 to-transparent" />
				...
				{/* ── Main Content ────────────────────────────────────────────── */}
			<main className="relative z-10 container mx-auto px-4 sm:px-6 py-8 max-w-5xl">
				{!hasProfile ? (
					<div className="flex flex-col items-center justify-center py-20">
						...
						<Button
							onClick={() => { 
								setActInfo({ actName: "", leadContact: "", expectedTravelers: 0, needs: EMPTY_NEEDS });
								setHasProfile(true); 
								setEditingAct(false); 
							}}
				...
				>
					<div className="flex items-center justify-between mb-5">
						<div className="flex items-center gap-3">
							...
							<h2 className="text-lg font-semibold text-white">Act Information</h2>
						</div>
					...
					{!editingAct ? (
						<>
							<div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
								<DetailCell label="ACT NAME" value={actInfo.actName || "—"} />
								<DetailCell label="LEAD CONTACT" value={actInfo.leadContact || "—"} />
								<DetailCell label="TOTAL TRAVELERS" value={actInfo.expectedTravelers.toString()} />
							</div>
							
							<div className="border-t border-purple-500/10 pt-5">
								<div className="flex items-center gap-2 mb-4">
									<Truck className="h-4 w-4 text-purple-400" />
									<h3 className="text-sm font-semibold text-white">Logistics Needs</h3>
								</div>
								<div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl">
									<div className="flex justify-between items-center text-sm">
										<span className="text-purple-300/70">Flights needed</span>
										<span className="text-white font-medium">{actInfo.needs.flights === null ? "—" : actInfo.needs.flights ? "Yes" : "No"}</span>
									</div>
									<div className="flex justify-between items-center text-sm">
										<span className="text-purple-300/70">Hotel needed</span>
										<span className="text-white font-medium">{actInfo.needs.hotel === null ? "—" : actInfo.needs.hotel ? "Yes" : "No"}</span>
									</div>
									<div className="flex justify-between items-center text-sm">
										<span className="text-purple-300/70">Transport needed</span>
										<span className="text-white font-medium">{actInfo.needs.transport === null ? "—" : actInfo.needs.transport ? "Yes" : "No"}</span>
									</div>
									<div className="flex justify-between items-center text-sm">
										<span className="text-purple-300/70">Visa support</span>
										<span className="text-white font-medium">{actInfo.needs.visa === null ? "—" : actInfo.needs.visa ? "Yes" : "No"}</span>
									</div>
								</div>
							</div>
						</>
							...
							<div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
								<div>
									...
									<Input
										...
										onChange={(e) => setActInfo((p) => ({ ...p, actName: e.target.value }))}
								...
								<div>
									...
									<Input
										value={actInfo.leadContact}
										onChange={(e) => setActInfo((p) => ({ ...p, leadContact: e.target.value }))}
										className="bg-white/5 border-purple-500/20 text-white placeholder:text-purple-300/30 rounded-xl h-11 focus-visible:ring-1 focus-visible:ring-purple-500 focus-visible:border-purple-500"
								...
								<div>
									...
									<Input
										type="number"
										min="0"
										value={actInfo.expectedTravelers}
										onChange={(e) => setActInfo((p) => ({ ...p, expectedTravelers: parseInt(e.target.value) || 0 }))}
										className="bg-white/5 border-purple-500/20 text-white rounded-xl h-11 focus-visible:ring-1 focus-visible:ring-purple-500 focus-visible:border-purple-500"
									/>
								</div>
							</div>

							<div className="border-t border-purple-500/10 pt-5 mb-6">
								<div className="flex items-center gap-2 mb-4">
									<Truck className="h-4 w-4 text-purple-400" />
									<h3 className="text-sm font-semibold text-white">Logistics Needs</h3>
								</div>
								<div className="grid grid-cols-1 sm:grid-cols-3 gap-5 max-w-3xl">
									<div className="flex flex-col gap-2">
										<span className="text-sm text-purple-300/70">Flights needed</span>
										<RadioSelect value={actInfo.needs.flights} onChange={(v) => setActInfo((p) => ({ ...p, needs: { ...p.needs, flights: v } }))} />
									</div>
									<div className="flex flex-col gap-2">
										<span className="text-sm text-purple-300/70">Hotel needed</span>
										<RadioSelect value={actInfo.needs.hotel} onChange={(v) => setActInfo((p) => ({ ...p, needs: { ...p.needs, hotel: v } }))} />
									</div>
									<div className="flex flex-col gap-2">
										<span className="text-sm text-purple-300/70">Transport needed</span>
										<RadioSelect value={actInfo.needs.transport} onChange={(v) => setActInfo((p) => ({ ...p, needs: { ...p.needs, transport: v } }))} />
									</div>
									<div className="flex flex-col gap-2">
										<span className="text-sm text-purple-300/70">Visa support</span>
										<RadioSelect value={actInfo.needs.visa} onChange={(v) => setActInfo((p) => ({ ...p, needs: { ...p.needs, visa: v } }))} />
									</div>
								</div>
							</div>
				...
				<section className="mb-8">
					<div className="flex items-center justify-between mb-5">
						<h2 className="text-lg font-semibold text-white">
							Travelers ({travelers.length} / {actInfo.expectedTravelers})
						</h2>
						{!showForm && (
							<Button
								onClick={openAddForm}
								disabled={!hasProfile || travelers.length >= actInfo.expectedTravelers || editingAct}
					...
					{travelers.length === 0 && !showForm && (
						...
						>
							...
							<Button
								onClick={openAddForm}
								disabled={!hasProfile || actInfo.expectedTravelers === 0 || editingAct}
							...
							{travelers.map((t) => (
								...
								>
									...
									<div className="flex flex-wrap gap-2">
										{t.passportCopyUrl ? (
											<Badge
												className="text-[10px] rounded-lg px-2 py-0.5 bg-green-500/15 text-green-300 border border-green-500/20 cursor-pointer hover:bg-green-500/25 transition-colors"
												onClick={() => window.open(t.passportCopyUrl, "_blank")}
											>
												<Check className="h-2.5 w-2.5 mr-1" />Passport ✓
											</Badge>
										) : (
											<Badge className="text-[10px] rounded-lg px-2 py-0.5 bg-yellow-500/15 text-yellow-300 border border-yellow-500/20">
												Passport pending
											</Badge>
										)}
										<Badge className="bg-indigo-500/15 text-indigo-300 border border-indigo-500/20 text-[10px] rounded-lg px-2 py-0.5">
											<UtensilsCrossed className="h-2.5 w-2.5 mr-1" />
											Diet: {t.dietaryRequirements || "No restrictions"}
										</Badge>
										{t.visaCopyUrl ? (
											<Badge
												className="text-[10px] rounded-lg px-2 py-0.5 bg-green-500/15 text-green-300 border border-green-500/20 cursor-pointer hover:bg-green-500/25 transition-colors"
											...
											>
												<Check className="h-2.5 w-2.5 mr-1" />Visa ✓
											</Badge>
										) : (
											t.visaNotes && (
												<Badge className="bg-amber-500/15 text-amber-300 border border-amber-500/20 text-[10px] rounded-lg px-2 py-0.5">
						...
						<div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm overflow-y-auto">
							...
							<div className="flex items-center gap-3">
								<Button
									...
									className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 rounded-xl gap-2 shadow-lg shadow-green-500/20 transition-all duration-300 hover:shadow-green-500/30"
								>
