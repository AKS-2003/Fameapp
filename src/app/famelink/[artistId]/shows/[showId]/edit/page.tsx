...
export default function EditShowPage() {
	...
	// Save handler — updates BaseShow via PUT /api/shows/[showId]
	const handleSave = async () => {
		...
		try {
			...
			// so the API won't reject empty strings for non-required fields
			const optStr = (val: string) => (val && val.trim()) || undefined;
			...
			const response = await fetch(`/api/shows/${showId}`, {
				...
				body: JSON.stringify({
					...
					style: optStr(formData.style),
					performanceType: optStr(formData.performance_type),
					duration: formData.performance_duration,
					biography: optStr(formData.biography),
					description: optStr(formData.biography),
					notes: optStr(formData.notes),
					isDraft: false,
					isPublic: true,
					realName: formData.real_name.trim(),
					email: formData.email.trim(),
					phone: formData.phone.trim(),
					managedBy: optStr(formData.managed_by),
					countryLiving: optStr(formData.country_living),
					homeCountry: optStr(formData.home_country),
					costumeColor: optStr(formData.costume_color),
					costumeColorTwo: formData.costume_color_two,
					costumeColorThree: formData.costume_color_three,
					customCostumeColor: optStr(formData.custom_costume_color),
					manualCostumeColor: optStr(formData.manual_costume_color),
					manualCostumeColorTwo: optStr(
						formData.manual_costume_color_two,
					),
					manualCostumeColorThree: optStr(
					...
					lightRequests: optStr(formData.light_requests),
					manualLightColor: optStr(formData.manual_light_color),
					manualLightColorTwo: optStr(
						formData.manual_light_color_two,
					),
					manualLightColorThree: optStr(
						formData.manual_light_color_three,
					),
					stagePositionStart: optStr(formData.stage_position_start),
					stagePositionEnd: optStr(formData.stage_position_end),
					customStagePosition: optStr(formData.custom_stage_position),
					profileImage,
					musicTrack: musicTrack.file_url ? musicTrack : null,
					galleryFiles,
					rehearsalVideo,
					techRider: optStr(formData.props_needed),
					equipment: optStr(formData.props_needed),
					showLink: optStr(formData.show_link),
					mcNotes: optStr(formData.mc_notes),
					stageManagerNotes: optStr(formData.stage_manager_notes),
					socialMedia: {
			...
			if (result.success) {
				...
				toast({
					title: "Error",
					description:
						result.error?.message || "Failed to update show",
					variant: "destructive",
				});
			}
		} catch {
	...
	if (loading) {
		return (
			<div className="min-h-screen bg-gradient-to-br from-purple-900 via-purple-800 to-pink-900 text-white flex items-center justify-center">
				...
				<p className="text-purple-200 ml-3">Loading show...</p>
			</div>
		);
	}
					...
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<p className="text-purple-100 text-sm">
								Update your show details
							</p>
						</div>
					</div>
					<Link href={`/famelink/${artistId}`}>
						<Button
							variant="ghost"
